...
unique_ptr の使用に関する経験則を適用できる。Exceptions prove the rule:（めったにない例外的なケースを目にすることで、普段はその存在を意識しないような「一般的なルール」の存在を再確認する、という意味の諺）
- ほとんどの場合、unique_ptr を参照引数として使用できる。その後、引数は他の参照と同様に動作する。さらに、「未定義状態」に nullptr を渡すこともでき、これは時に有益。
- 値パラメータとしての unique_ptr は呼び出し時にパラメータを「消費」する。関数に渡された変数はその後空になる。これは、オブジェクトが 1 つのパスだけを通過する必要がある場合に便利。これは、シンク (または排水管) デザインパターンの適用になる。
- 関数は戻り値に unique_ptr を返すことができる。これは、基本的にプロデューサー関数またはファクトリーとして、関数内で new されたオブジェクトをラップして返す場合に特に適している。
- ポインターが事前にすでに存在していた場合にのみ、 unique_ptr への参照を返す。自由関数から引数を返す、メソッドからはクラスのデータフィールドを返す等
- クラスのデータフィールドには removal について心配する必要がない unique_ptrs が適している。値と異なり unique_ptr は未定義フェーズがあり nullptr を store する。確かに、生のポインタよりも優れています。

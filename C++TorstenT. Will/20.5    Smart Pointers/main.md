...
- クラスのデータフィールドには removal について心配する必要がない unique_ptrs が適している。値と異なり unique_ptr は未定義の状態を持つことがあり、その歳は（中身に） nullptr を保存する。それは確かに（不定となる）生のポインタよりも優れている。
- unique_ptr を標準コンテナに入れることができまる。コンテナーは、(unique_ptr に)含まれるすべてのオブジェクトを所有する。別の場所から個々のコンテナ要素を参照する必要がある場合は、unique_ptr を参照することも、ラップされたオブジェクトへの raw のポインタとして参照することもできる。元のコンテナのいくつかの要素を別のコンテナから参照させたくない（コンテナは消える事があるので）場合に後者は useful。
